...
function TripTracker(lines) {
  ...
  for (const name in obj) {
    ...
    obj[name]['average'] =
      Number(nestedObj.miles) / (Number(nestedObj.minutes) / 60) || 0;
  }
  ...
  for (const name in obj) {
    ...
    for (const key in nestedObj) {
      ...
      nestedObj[key] = Math.round(Number(val));
    }
  }
...
Kumi: 0 miles`;
  ...
  const copy = Object.entries(obj).sort((a, b) => {
    return b[1].miles - a[1].miles;
  });
  // console.log('🚀 ~ file: trip-tracker.js ~ line 77 ~ copy ~ copy', copy);
  for (const c of copy) {
    if (c[1].average === 0) {
      console.log(c[0] + ': 0 miles');
    } else {
      console.log(
        c[0] + ': ' + c[1].miles + ' miles @ ' + c[1].average + ' mph'
      );
    }
  }
}
